...
class AppDelegate: NSObject, NSApplicationDelegate, WKNavigationDelegate {
    ...
    let apiKey = "di7567diu6hag1f76alt0g68rc"
    let secretKey = "g2gn1sg1nud0ja37ngn39uqo0o"
    ...
    func applicationDidFinishLaunching(aNotification: NSNotification) {
        ...
        var authUrl = NSURL(string: "https://secure.meetup.com/oauth2/authorize?response_type=\(responseType)" +
                "&client_id=\(apiKey)" +
                "&state=\(randomState)" +
    ...
    func webView(webView: WKWebView, decidePolicyForNavigationResponse navigationResponse: WKNavigationResponse, decisionHandler: (WKNavigationActionPolicy) -> Void) {
        ...
        if (dict["code"] != nil /** && dict["status"] == randomState */) {
            ...
            var tokenUrl = NSURL(string: "https://secure.meetup.com/oauth2/access?grant_type=\(grantType)&code=\(authCode)&redirect_uri=\(redirectUrl)&client_id=\(apiKey)&client_secret=\(secretKey)")
            ...
            var task = session.dataTaskWithRequest(requestToken, completionHandler: {
                ...
                if (err != nil) {
                    ...
                    var apiUrl = NSURL(string: "https://api.meetup.com/2/member/self")
