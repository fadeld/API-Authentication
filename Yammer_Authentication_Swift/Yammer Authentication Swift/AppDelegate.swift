...
class AppDelegate: NSObject, NSApplicationDelegate, WKNavigationDelegate {
    ...
    let apiKey = "0nWKEuXQ5hGMO4BfkQdjQ"
    let secretKey = "8oo0J034WFwtNBwQ9FBd4fN6wTr4f6wF0LrpqfvCo"
    ...
    func applicationDidFinishLaunching(aNotification: NSNotification) {
        ...
        var authUrl = NSURL(string: "https://www.yammer.com/oauth2/authorize?response_type=\(responseType)" +
                "&client_id=\(apiKey)" +
                "&state=\(randomState)" +
    ...
    func webView(webView: WKWebView, decidePolicyForNavigationResponse navigationResponse: WKNavigationResponse, decisionHandler: (WKNavigationActionPolicy) -> Void) {
        ...
        if (dict["code"] != nil /** && dict["status"] == randomState */) {
            ...
            var tokenUrl = NSURL(string: "https://www.yammer.com/oauth2/access_token?grant_type=\(grantType)&code=\(authCode)&redirect_uri=\(redirectUrl)&client_id=\(apiKey)&client_secret=\(secretKey)")
            ...
            var task = session.dataTaskWithRequest(requestToken, completionHandler: {
                ...
                if (err != nil) {
                    ...
                    var tokenDict: NSDictionary = jsonResult["access_token"] as NSDictionary
                    var token: NSString = tokenDict["token"] as NSString
                    println(token)

                    // Step 4: Make authenticated requests with the token
                    var apiUrl = NSURL(string: "https://www.yammer.com/api/v1/messages/following.json")
