//
//  AppDelegate.swift
//  Salesforce Authentication Swift
//
//  Created by SwiftBlog on 3/4/15.
...
class AppDelegate: NSObject, NSApplicationDelegate, WKNavigationDelegate {
    ...
    let apiKey = "3MVG9fMtCkV6eLhemYdmUB3rk5ohjaaC4urlGH9LNz3NPn5y2nzlehoWoTt3jyOUPuatCAkJM8gw8lGaSg6Bg"
    let secretKey = "3740815056308384220"
    ...
    func applicationDidFinishLaunching(aNotification: NSNotification) {
        ...
        var authUrl = NSURL(string: "https://login.salesforce.com/services/oauth2/authorize?response_type=\(responseType)" +
                "&client_id=\(apiKey)" +
                "&state=\(randomState)" +
    ...
    func webView(webView: WKWebView, decidePolicyForNavigationResponse navigationResponse: WKNavigationResponse, decisionHandler: (WKNavigationActionPolicy) -> Void) {
        ...
        if let items = urlComps!.queryItems as? [NSURLQueryItem] {
            ...
            if (dict["code"] != nil /** && dict["status"] == randomState */) {
                ...
                var tokenUrl = NSURL(string: "https://login.salesforce.com/services/oauth2/token?grant_type=\(grantType)&code=\(authCode)&redirect_uri=\(redirectUrl)&client_id=\(apiKey)&client_secret=\(secretKey)")
                ...
                var task = session.dataTaskWithRequest(requestToken, completionHandler: {
                    ...
                    if (err != nil) {
                        ...
                        var instanceUrl: NSString = jsonResult["instance_url"] as NSString
                        println(token)

                        // Step 4: Make authenticated requests with the token
                        var apiUrl = NSURL(string: "\(instanceUrl)/services/data/v20.0/")
                        ...
                        requestAuthenticated.addValue("application/json", forHTTPHeaderField: "Accept")
                        ...
                        var task = session.dataTaskWithRequest(requestAuthenticated, completionHandler: {
                            ...
                            var err2: NSError?
                            if let json = NSJSONSerialization.JSONObjectWithData(data, options: NSJSONReadingOptions.MutableContainers, error: &err2) as? NSDictionary {
                                if (err2 != nil) {
