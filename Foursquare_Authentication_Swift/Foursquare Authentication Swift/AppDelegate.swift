//
//  AppDelegate.swift
//  Foursquare Authentication Swift
//
...
class AppDelegate: NSObject, NSApplicationDelegate, WKNavigationDelegate {
    ...
    let apiKey = "YVJYDTDSZWNWPLZBSORX2PS33UYJZDDWZRUT54QSTLUISWNS"
    let secretKey = "ESZMWJG42CWPB5F3E3IS53BDPLQCXGLAMUDI4AXZI1FNUHNU"
    ...
    func applicationDidFinishLaunching(aNotification: NSNotification) {
        ...
        var authUrl = NSURL(string: "https://foursquare.com/oauth2/authenticate?response_type=\(responseType)" +
                "&client_id=\(apiKey)" +
                "&state=\(randomState)" +
    ...
    func webView(webView: WKWebView, decidePolicyForNavigationResponse navigationResponse: WKNavigationResponse, decisionHandler: (WKNavigationActionPolicy) -> Void) {
        ...
        if (dict["code"] != nil /** && dict["status"] == randomState */) {
            ...
            var tokenUrl = NSURL(string: "https://foursquare.com/oauth2/access_token?grant_type=\(grantType)&code=\(authCode)&redirect_uri=\(redirectUrl)&client_id=\(apiKey)&client_secret=\(secretKey)")
            ...
            var task = session.dataTaskWithRequest(requestToken, completionHandler: {
                ...
                if (err != nil) {
                    ...
                    var apiUrl = NSURL(string: "https://api.foursquare.com/v2/users/self/checkins?oauth_token=\(token)&v=20150101&m=foursquare")
