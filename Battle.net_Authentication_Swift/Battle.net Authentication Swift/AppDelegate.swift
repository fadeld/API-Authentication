//
//  AppDelegate.swift
//  Battle.net Authentication Swift
//
...
class AppDelegate: NSObject, NSApplicationDelegate, WKNavigationDelegate {
    ...
    func applicationDidFinishLaunching(aNotification: NSNotification) {
        ...
        var authUrl = NSURL(string: "https://us.battle.net/oauth/authorize?response_type=\(responseType)" +
                "&client_id=\(apiKey)" +
                "&state=\(randomState)" +
    ...
    func webView(webView: WKWebView, decidePolicyForNavigationResponse navigationResponse: WKNavigationResponse, decisionHandler: (WKNavigationActionPolicy) -> Void) {
        ...
        if (dict["code"] != nil /** && dict["status"] == randomState */) {
            ...
            var tokenUrl = NSURL(string: "https://us.battle.net/oauth/token?redirect_uri=\(redirectUrl)&grant_type=\(grantType)&code=\(authCode)&client_id=\(apiKey)&client_secret=\(secretKey)")
            ...
            var task = session.dataTaskWithRequest(requestToken, completionHandler: {
                ...
                if (err != nil) {
                    ...
                    var apiUrl = NSURL(string: "https://us.api.battle.net/account/user/id")
