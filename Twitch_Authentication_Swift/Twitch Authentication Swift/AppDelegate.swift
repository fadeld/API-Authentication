...
class AppDelegate: NSObject, NSApplicationDelegate, WKNavigationDelegate {
    ...
    let apiKey = "3ngmgogr3ayr819x8s6y9hxi8ytqx6o"
    let secretKey = "fs72nfnjud68gll60jbs0qo4dowdtkj"
    ...
    func applicationDidFinishLaunching(aNotification: NSNotification) {
        ...
        var authUrl = NSURL(string: "https://api.twitch.tv/kraken/oauth2/authorize?response_type=\(responseType)" +
                "&client_id=\(apiKey)" +
                "&state=\(randomState)" +
    ...
    func webView(webView: WKWebView, decidePolicyForNavigationResponse navigationResponse: WKNavigationResponse, decisionHandler: (WKNavigationActionPolicy) -> Void) {
        ...
        if (dict["code"] != nil /** && dict["status"] == randomState */) {
            ...
            var tokenUrl = NSURL(string: "https://api.twitch.tv/kraken/oauth2/token?grant_type=\(grantType)&code=\(authCode)&redirect_uri=\(redirectUrl)&client_id=\(apiKey)&client_secret=\(secretKey)")
            ...
            var task = session.dataTaskWithRequest(requestToken, completionHandler: {
                ...
                if (err != nil) {
                    ...
                    var apiUrl = NSURL(string: "https://api.twitch.tv/kraken/")
                    ...
                    requestAuthenticated.addValue("OAuth \(token)", forHTTPHeaderField: "Authorization")
